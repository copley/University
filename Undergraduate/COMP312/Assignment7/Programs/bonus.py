...
class Arrival(Process):
    ...
    def run(self, mu):
        ...
        G.departuremon.observe(now() - Arrival.lastDeparture)
        Arrival.lastDeparture = now()
...
def model(c, N, lamb, mu, maxtime, rvseed):
    ...
    W = G.delaymon.mean()
    DR = G.departuremon.mean()

    plt.clf()
    plt.hist(G.departuremon.yseries(), normed=True)
    plt.show()
...
for k in range(50):
    print k
    seed = 123*k
    result = model(c=1, N=10000, lamb=1, mu=1.00,
                  maxtime=2000000, rvseed=seed)
    allW.append(result[0])
    allDR.append(result[1])
#print allW
print ""
...
print ""
print "Estimate of DR:",numpy.mean(allDR)
print "Conf int of DR:",conf(allDR)
