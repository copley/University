...
public class ArrayQueue <E> extends AbstractQueue <E> implements BlockingQueue<E>
{
    ...
    // items arsizee stored from front..back-1
    // if front == back, then the queue is empty.

    // Used the folowing to help with the thread locking
    // http://grepcode.com/file/repository.grepcode.com/java/root/jdk/openjdk/6-b14/java/util/concurrent/ArrayBlockingQueue.java#ArrayBlockingQueue.remainingCapacity%28%29
    ...
    public ArrayQueue(int c, boolean fair)
    {
        com = c;
        data = (E[]) new Object[c];
    ...
    public int size () {
        // BUG: Was returning a size that was one less than it should of been
        return back - front;     
    }
    ...
    public E poll() {
        if (isEmpty())
            ...
            // BUG: The front was being incremented before it should of been
            E ans = data[front];
            data[front] = null;
            front = (front+1);
            return ans;
    ...
    public int remainingCapacity() 
    {
        ...
        l.lock();
        int r = data.length - size();
        l.unlock();

        return r;
    }

    public E poll(long timeout, TimeUnit unit) {return poll();}
    ...
    public void put(E e) throws InterruptedException
    {
        ...
        try
        {
            while( remainingCapacity() == 0 ) condition.await();
            condition.signal();
