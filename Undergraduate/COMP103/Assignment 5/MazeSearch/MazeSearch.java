...
public class MazeSearch {
    ...
    public MazeSearch(int size) {
        ...
        MazeGeneratorPrims generator = new MazeGeneratorPrims(size);
    ...
    private void explore(Cell cell) 
    {
        ...
        {
            drawCell(cell, Color.GREEN, true);
            finished = true;
            return;
        }

        // Set the current passage to yellow
        drawCell(cell, Color.YELLOW, true);
        cell.setVisited(true);

        // Explore the adjecent cells
        for( Cell c : neighbours.get(cell) ) 
        {
            explore(c);
            if( finished )
            {
        ...
        // Set the current cell to red
        drawCell(cell, Color.RED, true);
