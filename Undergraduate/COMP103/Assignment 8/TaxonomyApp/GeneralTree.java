...
public class GeneralTree
{
    ...
    public void addNode(String newName, String parentName) {
        if( root != null && root.findNode(newName) != null )
        {
            UI.println(newName + " is allready in the tree");
            return;
        }

        GeneralTreeNode n = new GeneralTreeNode(newName);
        if( parentName == null )
        {
            if( root != null ) n.addChild(root);
            root = n;
        }
        else
        {
            root.findNode(parentName).addChild(n);
        }
    }
    ...
    public void removeNode(String targetName) {
        if( root.getName() == targetName || root.findNode(targetName) == null ) return;
        else
        {
            GeneralTreeNode n = root.findNode(targetName);
            n.getParent().addChildrenFromNode(n);
            n.remove();
        }
    }
    ...
    public void moveSubtree(String targetName, String destinationName) {
        ...
        if( targetNode != root && !targetNode.contains(destinationNode) )
        {
            targetNode.remove();
    ...
    public String findClosestCommonAncestor(String target1, String target2) {
        if( root.findNode(target1) == null || root.findNode(target2) == null ) return null;

        GeneralTreeNode current = root.findNode(target1);

        while( current.findNode(target1) == null || current.findNode(target2) == null )
        {
    ...
    private void calculateLocations(int width)
    {
        root.setLocation(50, width+50, 50);
    }
