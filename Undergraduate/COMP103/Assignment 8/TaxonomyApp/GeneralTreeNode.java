...
public class GeneralTreeNode {
    ...
    public GeneralTreeNode findNode(String targetName) {
        if( targetName.equals(name) ) return this;

        for( GeneralTreeNode n : children ) 
        {
            GeneralTreeNode found = n.findNode(targetName);
    ...
    public void addChildrenFromNode(GeneralTreeNode donorNode) {
        for( GeneralTreeNode n : donorNode.getChildren() ) children.add(n); 
    }
    ...
    public void printSubtree(int depth) {
        for( int i = 0; i < depth; i++ ) UI.print(" ");
        UI.println(name);

        for( GeneralTreeNode n : children ) n.printSubtree(depth+1);
    }
    ...
    public boolean contains(GeneralTreeNode node) {
        ...
        for( GeneralTreeNode n : children ) if( n.contains(node) == true ) return true;
    ...
    {
        if( currentDepth == targetDepth ) UI.println(name);
        else if( currentDepth < targetDepth ) for( GeneralTreeNode n : children ) n.printAllAtDepth(targetDepth, currentDepth+1);

    }

    public void setLocation(int low, int high, int height)
    {
        location = new Location((high+low)/2, height);
        int xLow = low;
        int xPos = low;
        
        for( GeneralTreeNode n : children ) 
        {
            xPos += (high - low)/children.size();
            n.setLocation(xLow, xPos, height+100);
    ...
    public void redrawSubtree() {
        for( GeneralTreeNode n : children )
        {
            UI.drawLine(this.getLocation().getX(), this.getLocation().getY(), n.getLocation().getX(), n.getLocation().getY());
            n.redrawSubtree();
        }
