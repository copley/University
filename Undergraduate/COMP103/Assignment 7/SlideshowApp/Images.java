...
public class Images
{
    ...
    {
        ImageNode newImage = new ImageNode(imageFileName, null);
        if( head == null ) head = cursor = newImage;
        else cursor.insertAfter(newImage);
        //cursor.setNext(newImage);
    }
    ...
    {  
        ...
        {
            head.insertBefore(newImage, cursor);
        }
    }
    ...
    public void remove() {
        if( cursor == head && head != null ) cursor = head = head.getNext();
        else if( head.getNext() == null ) head = null;
        else 
        {
            ImageNode current = cursor;
            moveCursorLeft();
            
            current.removeNodeUsingPrevious(cursor);
    ...
    {
        if( head == null || head.count() <= 1 ) return;
        moveCursorToEnd();
        
        ImageNode newHead = cursor;
        ImageNode newCursor = newHead;
        moveCursorLeft();
        while( cursor != head )
        {
            ImageNode current = cursor;
            ...
            newCursor.insertAfter(current);
            newCursor = current;
        }
        
        newCursor.insertAfter(cursor);
    ...
    private void reverseRec() 
    {
        ...
        head = cursor = head.getNext().reverseUsingPrevious(head);
    }
}
