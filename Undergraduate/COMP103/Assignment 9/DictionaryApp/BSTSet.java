...
public class BSTSet <E extends Comparable<E>> extends AbstractSet <E> {
    ...
    public boolean add(E item) {
        if (item == null) throw new IllegalArgumentException("Null cannot be added to a BSTSet");
        if( root != null && root.contains(item) ) return false;    
    ...
    public boolean remove(E item) {
        if( root == null ) return false;
        else if( !root.contains(item) ) return false;
        else
        {
    ...
    private class BSTSetIterator implements Iterator<E> {
         ...
         */
        public E next() {
        	if( nodeStack.size() == 0 ) throw new NoSuchElementException();
        	
            BSTNode<E> n = nodeStack.pop();
