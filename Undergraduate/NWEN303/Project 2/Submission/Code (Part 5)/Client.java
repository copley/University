...
public class Client {
     ...
     */
    private static void submitJob(String ip, int port, long id, boolean result, String key) {
        Socket server;
        BufferedReader serverIn;
        PrintWriter serverOut;
     ...
     */
    public void run() {
        // Keep requesting jobs until we have found a solution or exausted the
        // key space
        while (!solutionFound && !exausted) {
            String[] job = requestJob(ip, port, chunkSize);

            // If the job is null then the server has shut down so exit
            if (job == null) {
                break;
            }

            long id = Long.parseLong(job[0]);
            byte[] cypherText = Blowfish.fromBase64(job[1]);
            String plainText = job[2];
            BigInteger key = new BigInteger(job[3]);
            int chunk = Integer.parseInt(job[4]);
            int keySize = Integer.parseInt(job[5]);

            // Process Job
            for (int i = 0; i < chunk && !solutionFound; i++) {
            ...
            submitJob(ip, port, id, solutionFound, (solutionFound ? key.toString() : null));
        }
    }
}
