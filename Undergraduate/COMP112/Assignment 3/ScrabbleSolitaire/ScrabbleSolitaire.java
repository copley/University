...
public class ScrabbleSolitaire implements UIMouseListener, UIButtonListener
{
    ...
    public void buttonPerformed(String button)
    {
        if( button.equals("Commit") && isRunning )
        {
            if( board.validPlay() )
            {
                ...
                if( !rack.isMoreTiles() )
                {
                    UI.println("No More Tiles In Bag");
                }
            }
    ...
    public void restart()
    {
        ...
        // If an error has occored set the isRunning flag to false so nothing will be drawn
        if( boardError || rackError ) isRunning = false;
        else draw();
