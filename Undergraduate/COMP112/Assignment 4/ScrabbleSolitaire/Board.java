...
{
    // Save Game Key
    private static final String SAVE_GAME_KEY_FIRSTTIME = "BOARD_FIRSTTIME";
    private static final String SAVE_GAME_KEY_BOARD = "BOARD_BOARD";
    private static final String SAVE_GAME_KEY_WORKING_TILES = "BOARD_WORKING";
    ...
    /**
     * Adds the board class to the save game
     */
    public void save( SaveGame s )
    {
        // Save the first time variable
        s.add(Board.SAVE_GAME_KEY_FIRSTTIME, String.format("%s", firstTime));
        ...
        for( int i = 0; i < board.length; i++ )
        {
            ...
            {
                data += String.format("%s-%s;", board[i][j].toString(), board[i][j].isCommited());
            }
            data += ";\n";
        }

        s.add(Board.SAVE_GAME_KEY_BOARD, data);

        // Save working tiles data
        data = "";

        for( int i = 0; i < workingTiles.size(); i++ )
        {
            data += String.format("[%d, %d];\n", workingTiles.get(i)[0], workingTiles.get(i)[1]);
        }

        s.add(Board.SAVE_GAME_KEY_WORKING_TILES, data);
    }
    
    /**
     * Loads all the information for the board class
     */
    public boolean load( SaveGame s )
    {
        // Should always contian the key but this is just to be safe
        if( s.containsKey(Board.SAVE_GAME_KEY_FIRSTTIME) && s.containsKey(Board.SAVE_GAME_KEY_BOARD) && s.containsKey(Board.SAVE_GAME_KEY_WORKING_TILES) )
        {
            // Get the first time variable
            String data = s.get(Board.SAVE_GAME_KEY_FIRSTTIME);

            if( data.equals("true") ) firstTime = true;
            else firstTime = false;

            // Load Board
            data = s.get(Board.SAVE_GAME_KEY_BOARD);
            ...
            for( int i = 0; i < rows.length; i++ )
            {
                ...
                {
                    board[i][j].reset();
                    String[] parsed = cols[j].split("-");

                    if( !parsed[0].equals("NULL") )
                    {

                        String[] info = parsed[0].substring(parsed[0].indexOf("[")+1, parsed[0].indexOf("]")).split(", ");
                        Tile t = new Tile(info[0], Integer.parseInt(info[1]));

                        board[i][j].setTile(t);
                        if(parsed[1].equals("true") ) board[i][j].commit();
                    }
                }
            }
            ...
            // Load Working Tiles
            workingTiles.clear();
            data = s.get(Board.SAVE_GAME_KEY_WORKING_TILES);
            ...
            if( !data.equals("") )
            {
                ...
                for( int i = 0; i < rows.length; i++ )
                {
                    ...
                    workingTiles.add(new int[] {Integer.parseInt(info[0]), Integer.parseInt(info[1])});
                }
            }
        }
        else return false;
    ...
    {
        return !board[row][col].containsTile();
    }
    ...
    {
        // Add all the tiles being commited to the log
        for( int i = 0; i < workingTiles.size(); i++ )
        {
            int x = workingTiles.get(i)[0];
            int y = workingTiles.get(i)[1];
            
            log.add(x, y, board[x][y].getLetter());
        }
        ...
        firstTime = false;
        workingTiles.clear(); //= new ArrayList<int[]>();
    }
    ...
    public boolean reset()
    {
        ...
        {
            Trace.println("Failed to open board file");
            Trace.println("Press the reset the button if you wish to try again");
            return true;
        }
    }
