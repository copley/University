...
public class Controller {
	...
	private List<RedrawListener> listeners = new ArrayList<>();
	...
	private Map<PlayerCharacter, String> playerNames;
	...
	public void init() {
		...
		// to the rooms?
		cardsPerPlayer = cards.size() / characters.size();
	...
	/**
	 * Returns the name associated with a player
	 * 
	 * @param p Player we want the name of
	 * @return String players name
	 */
	public String getPlayerName(Player p) {
		return playerNames.get(p.getPlayerCharacter());
	}
	 ...
	 */
	public void addPlayerCharacter(PlayerCharacter character, String playerName) {
		...
		playerNames.put(character, playerName);
	}
	 ...
	 * 
	 * @return the board contained in the controller
	 */
	...
	/**
	 * Add a redraw listener
	 * 
	 * @param r listener to be added 
	 */
	public synchronized void addRedrawListener(RedrawListener r) {
		listeners.add(r);	
	}
	
	/**
	 * Remove a redraw listener
	 * 
	 * @param r listener to be removed
	 */
	public synchronized void removeRedrawListener(RedrawListener r) {
		listeners.remove(r);
	}
	
	/**
	 * Fire an event making all the listeners redraw there UI
	 */
	public synchronized void fireRedrawEvent() {
		RedrawEvent e = new RedrawEvent(this);
		
		Iterator<RedrawListener> i = listeners.iterator();
		
		while(i.hasNext()) {
			((RedrawListener) i.next()).onRedraw(e);
		}
	}

	/**
	 * Reset the controller, remove all players
	 * and set num players to -1
	 */
	public void reset() {
		characters.clear();
		numPlayers = -1;	
