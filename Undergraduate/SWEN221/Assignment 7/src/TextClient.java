...
public class TextClient {
	...
	private static ArrayList<Player> inputPlayers(int nplayers, Location go) {
		...
		for (int i = 0; i != nplayers; ++i) {
			...
			String token = inputString("Player #" + i + " token?");
			if (!tokens.contains(token)) {
				...
				System.out.println();
				token = inputString("Player #" + i + " token?");
			}
			tokens.remove(token);
	...
	private static void playerOptions(Player player, GameOfMonopoly board) {
		...
		System.out.println("* Buy House or Sell House [NOT SUPPORTED]");
	...
	public static void main(String args[]) {
		...
		System.out.println("By David J. Pearce, 2007");
