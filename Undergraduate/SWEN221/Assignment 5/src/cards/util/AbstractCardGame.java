...
public abstract class AbstractCardGame implements CardGame {
	...
	public CardGame clone() {
		AbstractCardGame g = null;
		
		// Create a new card game
		if (this instanceof ClassicWhist) {
			g = new ClassicWhist();
		} else if (this instanceof KnockOutWhist) {
			g = new KnockOutWhist();
		} else if (this instanceof SingleHandWhist) {
			g = new SingleHandWhist();
		}
		
		// Clone the players
		for (Player.Direction d : players.keySet()) {
			g.players.put(d, players.get(d).clone());
		}
		
		// Clone the tricks
		for (Player.Direction d : tricks.keySet()) {
			g.tricks.put(d, tricks.get(d).intValue());
		}
		
		// Clone the scores
		for (Player.Direction d : scores.keySet()) {
			g.scores.put(d, scores.get(d).intValue());
		}
		
		// Can just copy trumps
		g.trumps = trumps;
		
		// And finally set the current trick
		g.currentTrick = (currentTrick == null) ? null : currentTrick.clone();
