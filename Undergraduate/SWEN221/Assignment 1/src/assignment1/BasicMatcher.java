...
public class BasicMatcher implements Matcher {
	...
	public boolean match(String text, String regex) {
		...
		} while (text.length() != 0 && (matchChar(text,regex) || text.indexOf(regex.charAt(0)) != -1));
		return false;
	}
	...
	protected boolean matchHere(String text, String regex) {
		...
		if (regex.length() == 1 && text.length() == 1) {
		...
		if (matchChar(text, regex)) {
			//String newRegex = regex.length() > 1 ? regex.substring(1) : "";
			
			//return matchHere(newText, newRegex);
	...
	protected boolean matchStar(String text, String regex) {
		String restOfRegex = regex.length() > 2 ? regex.substring(2) : "";
	...
	/**
	 * Takes a regex and text string, assuming the regex string starts with a letter folowed by a
	 * '+'. It will try to match the charactor to the text string 1 or more times
	 * @param text
	 * @param regex
	 * @return if the match is successful, and false if it isnt
	 */
	protected boolean matchPlus(String text, String regex) {
		// If the length of the text is 0 then the regex can not match as
		// with the '+' the charactor must match 1 or more times
		if (text.length() == 0) {
			return false;
		}
		...
		// Match the charactor as many times as possible
		while (matchChar(text, regex)) {
			text = text.substring(1);
	 ...
	 * <p>
	 * This method is not used by the above matching code, and you must NOT use
	 * it in your solution! However, you may find it helpful for testing.
	 * 
	 * @param text
