...
Export our module so the test script can access our functions
...
The reflect tree operation works nicely as a fold
...
If our start vertex is equal to the finish vertex then return an empty list of
paths (as the path from a vertex to its self contains no edges)
